...
setwd("~/Google-Drives/dr.john.urban/Carnegie/Projects/Bcop_v2_release/hic/inversion-X-aln/laveAnalysis/Oct2024/otherDots/IV/male/")
...
# ii.links.male <- read.extra.short.pe(fh="../../../../July2024/XO-phasemap-q0.extrashort.sorted.chromosome-II.txt.gz")
# iii.links.male <- read.extra.short.pe(fh="../../../../July2024/XO-phasemap-q0.extrashort.sorted.chromosome-III.txt.gz")
iv.links.male <- read.extra.short.pe(fh="../../../../July2024/XO-phasemap-q0.extrashort.sorted.chromosome-IV.txt.gz")

##############################################################
## DUMMY VARIABLE FOR XLINKS TO MAKE THIS EASIER
##############################################################
# xlinks <- xlinks.female
# xlinks <- xlinks.male ; tig_size <- Xlen
# xlinks <- ii.links.male ; tig_size <- IIlen
# xlinks <- iii.links.male ; tig_size <- IIIlen
xlinks <- iv.links.male ; tig_size <- IVlen
dim(xlinks)
head(xlinks)

############################################################################################################################
## PARAM PICKING -- OLD FBR NOTES FOR BCOPV2 PAPER - CAN BE DELETED FOR NEW TEMPLATES
############################################################################################################################
...
# origParams.adj1.male.alt   <- list(MAPQ=10, bx=250000, k=12, gridsize=1024, minpair=1e6, maxpair=NA, quantile_thresh=0.999, minRegionSeparation=1e6, clust.method="h")
...
# other.dots.male   <- list(MAPQ=10, bx=50e3, k=3, gridsize=2048, minpair=1e6, maxpair=10e6, quantile_thresh=0.9999, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.

# other.dots.male   <- list(MAPQ=10, bx=50e3, k=5, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.9995, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.

# other.dots.male   <- list(MAPQ=10, bx=50e3, k=7, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.999, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.

# other.dots.male   <- list(MAPQ=10, bx=50e3, k=7, gridsize=2048, minpair=4e6, maxpair=NA, quantile_thresh=0.999, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.

# iv.dots.male.depr1   <- list(MAPQ=10, bx=50e3, k=2, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.9999, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.
# iv.dots.male.depr2   <- list(MAPQ=10, bx=100e3, k=2, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.99995, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.

iv.dots.male   <- list(MAPQ=10, bx=100e3, k=1, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.99995, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.

# ii.dots.male.interesting.1   <- list(MAPQ=10, bx=50e3, k=12, gridsize=2048, minpair=1e6, maxpair=NA, quantile_thresh=0.9999, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.
# ii.dots.male.interesting.2   <- list(MAPQ=10, bx=100e3, k=12, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.9999, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.
# 
# iii.dots.male   <- list(MAPQ=10, bx=100e3, k=1, gridsize=2048, minpair=1e6, maxpair=NA, quantile_thresh=0.99995, minRegionSeparation=1e6, clust.method="h") ## 2024-10-24-v1; same results as narrower.FBR.2.Params; but is less aggressive.
...
# useParams <- other.dots.male
useParams <- iv.dots.male
# useParams <- ii.dots.male
# useParams <- iii.dots.male
# useParams <- origParams.adj1.male.alt
useParams
...
df.complete.identical.dist.stats <- plotDensityOfInteractionRatios(df.complete.identical.dist[df.complete.identical.dist$FE <= FE.upper.limit,], 
                                                                   transform.fxn = log10, 
                                                                   add.legend = TRUE, 
