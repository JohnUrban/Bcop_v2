...
setwd("~/Path/To/Desired/Working/Dir/")
...
## SCOPE FUNCTIONS: source("/Path/To/SCOPE-v1.20250307.R")
## Relative path from 02-template dir where this template lives.
source("../01-functions/SCOPE-v1.20250307.R")



############################################################################################################################
############################################################################################################################
## SETTING UP FOR SCOPE PIPELINE.
############################################################################################################################
############################################################################################################################
...
##############################################################
## READ IN HIC DATA (or other paired-end data in Juicer extrashort format)
##############################################################
hic.sample.1 <- read.extra.short.pe(fh="../../../../July2024/XO-phasemap-q0.extrashort.sorted.X-chromosome-only.txt.gz")


##############################################################
## DUMMY VARIABLE FOR HIC DATA, CALLED "XLINKS"
## - This allows you to try many "hic samples" above with the same variable used throughout.
##############################################################
# Set dummy varuable to desired hic sample.
xlinks <- hic.sample.1
...
## CHR VARIABLE IS NEED FOR WRITING BED, BEDPE, AND OTHER TABLE FILES.
CHR <- xlinks$chr1[1]

## Check
CHR

## NOTE: SCOPE expects the txt file to contain only self-self (e.g. intrachromosomal) read pairs; i.e. chr1=chr2.
##     : - if your data is more complex, you can subset it here.
##     : - first uncomment "CHR" on the next line, and set it to the sequence name you want to look at.
##     : - then uncomment the subsequent lines and do the subsetting.
# CHR <- "X"
# xlinks <- xlinks[xlinks$chr1 == CHR & xlinks$chr2 == CHR, ]
# rownames(xlinks) <- 1:dim(xlinks)[1]
# ## Check
# CHR
# dim(xlinks)
# head(xlinks)
...
##############################################################
## CHR LEN (i.e. "tig_size")
##############################################################
tig_size <- NA ## Set this to the length of "CHR"; i.e. the contig, scaffold, or chromosome you are analyzing.
...
############################################################################################################################
## PARAM PICKING 
## - Below are some notes I wrote to myself as developing and exploring.
############################################################################################################################
############################################################################################################################
...
############################################################################################################################
## JOHNS EXAMPLES OF VARIOUS PARAMETER SETTINGS - CAN BE DELETED FOR CLEAN TEMPLATE, LEAVING JUST ONE GENERIC SETTING.
############################################################################################################################
origParams.female <- list(MAPQ=10, bx=250000, k=4, gridsize=1024, minpair=1e6, maxpair=NA, quantile_thresh=0.999, minRegionSeparation=5e6, clust.method="h")
origParams.male   <- list(MAPQ=10, bx=250000, k=3, gridsize=1024, minpair=1e6, maxpair=NA, quantile_thresh=0.999, minRegionSeparation=5e6, clust.method="h")
...
narrower.FBR.4.Params.female <- list(MAPQ=10, bx=50e3, k=4, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.9999, minRegionSeparation=5e6, clust.method="h") 
narrower.FBR.4.Params.male   <- list(MAPQ=10, bx=50e3, k=3, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.9999, minRegionSeparation=5e6, clust.method="h") 

# Set your desired parameters here.
my.scope.parameters <- list(MAPQ=10, bx=50e3, k=4, gridsize=2048, minpair=5e6, maxpair=NA, quantile_thresh=0.9999, minRegionSeparation=5e6, clust.method="h") 
...
###############################################################################
## PIPELINE
## - The below pipeline automatically created PDF files in SCOPE-figures/ and table files (e.g. BED, BEDPE) in SCOPE-tables/.
## - If you want to see the plots in Rstudio without writing to pdf, just avoid the pdf() and dev.off() calls.
###############################################################################
system("mkdir -p SCOPE-figures")
...
# ## - This is now learned or hard-coded at the top of the file, but feel free to change it here.
# CHR <- "X"
